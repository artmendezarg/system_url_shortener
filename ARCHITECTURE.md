...
- **Commits:** Conventional Commits + trailer `Co-Authored-By: Claude ...` en cada commit generado con asistencia de IA. Regla dura: nunca `git commit --amend` sobre un commit de la IA después de un ajuste humano — siempre un commit nuevo, para que el diff "propuesto por la IA vs. corregido por el ingeniero" quede visible en el historial para siempre.
- **Disparo del trabajo:** manual, dentro del Codespace. El ingeniero decide cuándo invocar a Claude Code para cada tarea, revisa el diff localmente antes de subirlo. Se descarta como método principal la automatización vía GitHub Action (`@claude` en un issue abriendo el PR por sí sola) porque el rubro del ejercicio pide explícitamente ejecución liderada por el ingeniero ("engineer-led execution accelerated by AI, not autonomous orchestration"); queda documentada como capacidad adicional disponible, usada puntualmente y siempre con merge manual, no como el flujo por defecto.
- **Plantilla de PR** (`.github/PULL_REQUEST_TEMPLATE.md`) con secciones fijas: tarea/intención original, prompt(s) usados, resumen de lo generado por la IA, resultado de los quality gates (build, tests, lint, dependency scan — sección 11), y un campo obligatorio **"Decisión del ingeniero"** con tres casos:
  - *Aceptado:* se aprueba y mergea tal cual.
  ...
  - *Ajustado:* se agrega un commit nuevo sobre la misma rama (de la IA corrigiendo algo puntual, o del ingeniero editando directo); el PR conserva ambos commits visibles, nunca se reescribe el original.
- **Branch protection en `main`:** push directo bloqueado; merge solo vía PR con el pipeline de CI en verde (sección 11). Con un repositorio de una sola persona esto no fuerza un segundo revisor humano — fuerza el *proceso* (PR obligatorio + checks en verde), que es lo mecánicamente verificable aquí.
