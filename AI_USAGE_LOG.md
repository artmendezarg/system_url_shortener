...
## 2026-09-04 — [Fix] PR #20 — `guest` de RabbitMQ rechazado por restricción `loopback_users`

- **Prompt:** el usuario corrió `mvn verify` en `v1-legacy-monolith` (Codespace) tras el fix anterior: compiló, pero `ClickEventPublishingIntegrationTest` falló con `AmqpIO java.io.IOException` en sus 2 tests (publicar evento y no-publicar-si-expiró), mientras que **todos los demás 19 tests pasaron**, incluidos `ClickEventPublishingResilienceTest` (RabbitMQ inalcanzable) y las 3 suites previas de caracterización/integración — señal clara de que el problema era específico de esta clase, no una regresión general.
- **Diagnóstico:** el usuario "guest" por defecto de RabbitMQ solo puede conectarse desde loopback real (restricción `loopback_users` del broker). Una conexión a través del puerto mapeado de Testcontainers no se percibe como loopback desde la perspectiva del broker (dentro del contenedor, la conexión llega vía la red de Docker, no vía `127.0.0.1` real), así que el handshake AMQP se corta a mitad de camino — el cliente ve un `IOException` de bajo nivel en vez de un error de autenticación limpio. Es un problema documentado de Testcontainers + RabbitMQ con las credenciales por defecto.
- **Fix aplicado:** se agregó `.withUser("appuser", "appuser_local")` a la definición del contenedor de prueba, creando un usuario dedicado sin la restricción de `guest`. El resto del código (`getAdminUsername()`/`getAdminPassword()` en `@DynamicPropertySource`) no cambió — esos métodos ahora reflejan el usuario nuevo automáticamente.
- **Decisión:** Ajustado — corregido en la misma rama (`feature/click-events-rabbitmq`).
- **Nota positiva:** este fallo, aunque frustrante, confirmó indirectamente que el diseño de resiliencia funciona: `ClickEventPublishingResilienceTest` pasó sin problema con RabbitMQ genuinamente inalcanzable, y el fallo de conexión al broker real (por credenciales, no por caída del servicio) quedó contenido exactamente donde se esperaba — en la prueba que intenta *leer* la cola directamente, no en el camino de redirección de la aplicación.

## 2026-09-04 — [Fix] PR #20 — Cola `click-events` nunca se declaraba en el broker de prueba

- **Prompt:** el usuario compartió el stack trace completo (no solo el resumen) del error anterior: `com.rabbitmq.client.ShutdownSignalException: ... 404, reply-text=NOT_FOUND - no queue 'click-events' in vhost '/'`. Esto reveló que la conexión al broker SÍ funcionaba (el fix anterior del usuario `guest`/loopback fue correcto) — el problema real era que la cola nunca llegó a existir.
- **Diagnóstico:** el bean `Queue` declarado dentro de un `@TestConfiguration` anidado (`TestQueueConfig`) nunca se registró en el `ApplicationContext`. Motivo: `@SpringBootTest(classes = V1LegacyMonolithApplication.class, ...)` usa el atributo `classes` explícito, lo que desactiva la auto-detección de clases de configuración anidadas que Spring Boot Test normalmente aplica cuando no se especifica `classes`. El bean simplemente nunca se creó, así que `RabbitAdmin` no tenía nada que declarar.
- **Fix aplicado:** se eliminó el `@TestConfiguration` anidado y se reemplazó por una declaración imperativa y explícita: se inyecta `RabbitAdmin` (bean auto-configurado por Spring Boot) y se llama `rabbitAdmin.declareQueue(new Queue(...))` en un método `@BeforeEach`. Esto elimina cualquier ambigüedad sobre detección automática de configuración — la declaración ocurre siempre, de forma explícita, antes de cada prueba (idempotente).
- **Decisión:** Ajustado — corregido en la misma rama (`feature/click-events-rabbitmq`).
- **Lección:** el mensaje resumido de Maven/Surefire (`AmqpIO java.io.IOException`) no era suficiente para diagnosticar correctamente — llevó a un primer fix real pero incompleto (el de `guest`/loopback, que sí hacía falta pero no era la causa raíz completa). El stack trace completo con la causa anidada (`ShutdownSignalException` con el código de razón AMQP 404) fue lo que reveló el problema real. Vale la pena pedir el stack trace completo desde el principio en vez del resumen cuando un error de infraestructura (no de lógica de negocio) está en juego.

## 2026-09-04 — [Fix] PR #20 — SpotBugs `EI_EXPOSE_REP2` en `ClickEventPublisher`

- **Prompt:** el usuario corrió `mvn verify` tras el fix anterior — todos los tests pasaron esta vez (confirmando que la declaración explícita de la cola vía `RabbitAdmin` funcionó), pero el build falló en un gate distinto: `spotbugs:check` reportó `EI_EXPOSE_REP2` en `ClickEventPublisher`: "may expose internal representation by storing an externally mutable object into ClickEventPublisher.rabbitTemplate".
- **Diagnóstico:** SpotBugs marca como riesgo guardar una referencia a una clase concreta mutable recibida por constructor directamente en un campo. `RabbitTemplate` es una clase concreta con muchos setters (mutable). No es un hallazgo falso — es una oportunidad real de mejora, no solo ruido a silenciar.
- **Fix aplicado:** se cambió el tipo del campo/parámetro de `RabbitTemplate` (clase concreta) a `AmqpTemplate` (la interfaz que `RabbitTemplate` implementa). Spring sigue inyectando el mismo bean auto-configurado sin ningún cambio de comportamiento, pero SpotBugs no marca campos de tipo interfaz con este patrón — la misma razón, ahora explícita, por la que `UrlRecordRepository` (interfaz de Spring Data) en otras clases del proyecto nunca disparó este hallazgo. Se prefirió este fix sobre silenciar el hallazgo con `@SuppressFBWarnings`: es una mejora real de diseño (programar contra la interfaz, no la implementación), no solo pasar el gate.
- **Decisión:** Ajustado — corregido en la misma rama (`feature/click-events-rabbitmq`).
...
- **Verification performed:** `git diff --stat` confirms the exact file list touched; `python3 -c "import xml.etree.ElementTree"` / `import yaml` re-parsed every modified XML/YAML file to confirm structural validity; the whole repo was grepped for remaining Spanish accented characters (`áéíóúñÁÉÍÓÚÑ¿¡`) in `.java`/`.yml`/`.xml`/`.env.example` files — none remain.
- **Decision:** pending engineer review (see PR). Follow-up work (not in this PR): translating this log's own pre-existing entries and the historical PR descriptions (#1 through #20) to English — tracked separately.
